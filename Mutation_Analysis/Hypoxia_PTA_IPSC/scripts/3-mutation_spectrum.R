...
##### Table S1
# type_occur_S1 <- type_occur_raw %>% filter(row.names(.) %in% sSNV_SCAN2_df$Cell_ID) %>% 
#   mutate(ture_call_sum = rowSums(select(., 1:6)), Cell_ID = row.names(.))
# sSNV_SCAN2_df_S1 <- sSNV_SCAN2_df[, c("Cell_ID", "Condition", "snv.rate.per.gb", "snv.burden", "Coverage", "Depth", "MAPD", "CoV")] %>%
#   merge(type_occur_S1) %>% arrange(match(Cell_ID, Cell_ID_list))
# write.csv(sSNV_SCAN2_df_S1, paste0(table_dir, "/3-Table_S1_IPSC.csv"), row.names = F)
# type_occurrences$Condition <- metadata_df$Condition

##### calculate estimated burden (per GB, age matched)
type_occur_raw <- type_occur_raw %>% filter(row.names(.) %in% Cell_ID_list) %>% 
  mutate(across(everything(), ~ . / factor_hg37), Condition = metadata_df$Condition)
est_ssnv_burden <- sSNV_SCAN2_df$snv.rate.per.gb
...
est_total_ssnv_ctrl <- sum(est_ssnv_burden[ctrl_range])
est_total_ssnv_dis <- sum(est_ssnv_burden[dis_range])
called_total_ssnv_ctrl <- sum(type_occur_est$true_call_sum[ctrl_range])
called_total_ssnv_dis <- sum(type_occur_est$true_call_sum[dis_range])
...
type_occur_est[ctrl_range, 1:8] <- type_occur_est[ctrl_range, 1:8] * ctrl_ratio
type_occur_est[dis_range, 1:8] <- type_occur_est[dis_range, 1:8] * dis_ratio
...
for (plot_type in plot_type_list) {
  if (plot_type == "raw") {
    ...
    type_avg_per_cell_diff <- colMeans(type_occur_plot[dis_range, 1:8]) - colMeans(type_occur_plot[ctrl_range, 1:8])
    type_occur_plot[nrow(type_occur_plot) + 1, 1:8] <- type_avg_per_cell_diff
    type_occur_plot[[9]] <- as.character(type_occur_plot[[9]])
    type_occur_plot[nrow(type_occur_plot), 9] <- "Net change (Hypoxia - Normoxia)"
    ...
    p_type_occ_raw <- plot_spectrum_customize(type_occur_plot, by = type_occur_plot$Condition, CT = TRUE, indv_points = TRUE, error_bars = "none") +
      facet_wrap(factor(by, unique(type_occur_plot$Condition)) ~ .) + labs(y = "sSNV rate per GB") + theme_linedraw() +
    ...
    p_type_occur_est <- plot_spectrum_absolute(type_occur_plot, by = type_occur_plot$Condition, CT = TRUE, indv_points = TRUE, error_bars = "none") +
      facet_wrap(factor(by, unique(type_occur_plot$Condition)) ~ total_mutations) + labs(y = "sSNV rate per GB") + scale_y_continuous(breaks = c(0, 25, 50, 75, 100)) + theme_linedraw() + 
      theme(panel.background = element_rect(fill = "white"), panel.grid.major = element_line(color = "grey80", linetype = "dashed", linewidth = 0.25), panel.grid.minor = element_blank(), 
            panel.border = element_rect(linewidth = 0.5), text = element_text(size = 15), axis.ticks.x = element_blank())
  ...
  barplot_type_occur <- ggbarplot(melt_type_occur_test, x = "type", y = "no_mutation", color = "Condition", label = TRUE, lab.size = 6, 
    ...
    stat_compare_means(aes(group = Condition, label = sprintf("p = %1.2f", as.numeric(..p.format..))), label.y = 1.04 * max(melt_type_occur_test$no_mutation), size = 6) +
