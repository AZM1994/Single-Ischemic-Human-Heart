...
# mut_mat_denovo <- t(mut_mat_raw_AMG) + 0.0001
mut_mat_denovo <- t(mut_mat_est_AMG) + 0.0001
# mut_mat_denovo <- t(mut_mat_est) + 0.0001
# mut_mat_denovo <- t(mut_mat_raw) + 0.0001
...
nmf_res_4_sigs <- nmf_res_4_sigs[, c(3, 2, 1, 4)]
...
pdf(paste0(suppl_figure_dir, "/4-lmer_denovo_sigs.pdf"), width = 12, height = 8)
  for (i in c(1 : (ncol(sigNet_denovo_contri_nor) - 5))){
    ...
    burden_df <- sigNet_denovo_contri_nor[c(refined_selected_sigs_list[i], "Age", "Condition", "Case_ID", "Color")] %>% setNames(c("snv.rate.per.gb", "Age", "Condition", "Case_ID", "Color")) %>% 
      mutate(Condition = relevel(factor(Condition), ref = "Control")) %>% group_by(factor(Condition, levels = c(dis_name, ctrl_name))) %>% arrange(Age, .by_group = TRUE)
    ...
    p_SNV_burden_lme <- ggplot(burden_df, aes(x = Age, y = snv.rate.per.gb)) + 
      ...
      # annotate("text", size = 6, x = 0, y = 0.80 * max_y, label = paste0("aging effect:", format(round(fixef(burden_age_model_control)[2], digits = sig_digits), nsmall = sig_digits), "/(GB.year), ",
