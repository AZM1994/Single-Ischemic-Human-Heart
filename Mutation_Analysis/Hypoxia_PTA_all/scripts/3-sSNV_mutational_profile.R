...
write.csv(mut_mat_raw, file = paste0(table_dir, "/3-mut_mat_raw_PTA_all.csv"))
...
write.csv(mut_mat_raw_AMG_cond, file = paste0(table_dir, "/3-mut_mat_raw_AMG_cond_PTA_all.csv"))
...
write.csv(mut_mat_est, file = paste0(table_dir, "/3-mut_mat_est_PTA_all.csv"))
mut_mat_est_AMG <- mut_mat_est[Cell_ID_list_AMG, ]
mut_mat_est_AMG_cond <- mut_mat_est_AMG %>% t() %>% as.data.frame() %>% 
  mutate(Control = rowMeans(.[, ctrl_range_AMG]), IHD = rowMeans(.[, dis_range_AMG]), 
         Net_change = pmax(IHD - Control, 0, na.rm = TRUE))
write.csv(mut_mat_est_AMG_cond, file = paste0(table_dir, "/3-mut_mat_est_AMG_cond_PTA_all.csv"))
...
ggsave(paste0(other_figure_dir, "/3-sSNV_mutational_profile_raw_relative.pdf"), plot = p_96_raw_rel, width = 10, height = 4.25, dpi = 600)

p_96_est_abs <- plot_96_profile_abs(mut_mat_est_AMG_cond[, c(ctrl_name, dis_name, "Net_change")]) + theme_linedraw() + 
  theme(panel.background = element_rect(fill = "white"), panel.grid.major = element_line(color = "grey80", linetype = "dashed", linewidth = 0.15), panel.grid.minor = element_blank(), 
        panel.border = element_rect(linewidth = 0.5), text = element_text(size = 10), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
