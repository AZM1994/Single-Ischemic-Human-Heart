...
p_original_vs_revised_burden_snv <- ggplot(snv_matrix, aes(x = original, y = revised)) + 
  ...
  theme(panel.background = element_rect(fill = "white"), panel.grid.major = element_line(color = "grey80", linetype = "dashed", linewidth = 0.25), 
        panel.grid.minor = element_blank(), panel.border = element_rect(size = 0.5), text = element_text(size = 12), plot.margin = margin(t = 5, r = 15, b = 5, l = 5))
...
revised_df_subset <- revised_df[c(2,6,58,59,75,76),]
revised_df_subset$data_source <- c("recovered missing burden", "modified SCAN2 burden", "recovered missing burden", "modified SCAN2 burden", "modified SCAN2 burden", "recovered missing burden")
p_recovered_missing_burden_snv <- ggplot(revised_df_subset, aes(x = factor(Cell_ID, level = Cell_ID_list), y = snv.rate.per.gb, shape = data_source, color = data_source)) + 
  ...
  theme(panel.background = element_rect(fill = "white"), panel.grid.major = element_line(color = "grey80", linetype = "dashed", linewidth = 0.25), 
        ...
        legend.position=c(0.75, 0.8), legend.background = element_rect(fill = "grey90"))
