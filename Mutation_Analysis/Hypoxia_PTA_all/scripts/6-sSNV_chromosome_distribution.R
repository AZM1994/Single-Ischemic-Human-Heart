...
summary_normal_df <- vcf_df_normal %>% 
  ...
  arrange(match(Cell_ID, sSNV_SCAN2_df_AMG$Cell_ID)) %>% 
  mutate(percentage = replace(percentage, is.na(percentage), 0)) %>% 
  mutate(est_snv_burden = rep(sSNV_SCAN2_df_AMG$snv.rate.per.gb[sSNV_SCAN2_df_AMG$Condition == ctrl_name], each = 22)) %>% 
...
summary_disease_df <- vcf_df_disease %>% 
  ...
  arrange(match(Cell_ID, sSNV_SCAN2_df_AMG$Cell_ID)) %>% 
  mutate(percentage = replace(percentage, is.na(percentage), 0)) %>% 
  mutate(est_snv_burden = rep(sSNV_SCAN2_df_AMG$snv.rate.per.gb[sSNV_SCAN2_df_AMG$Condition == dis_name], each = 22)) %>% 
...
p_chr_percentage <- ggplot(merged_avg_df, aes(x = avg_percentage.x, y = avg_percentage.y, color = percentage_p.value_list)) +
  ...
  scale_x_continuous("avg. chr sSNV percentage in Control") + scale_y_continuous("avg. chr sSNV percentage in IHD") + 
...
ggsave(paste0(other_figure_dir, "/6-sSNV_chr_percentage_by_condition.pdf"), plot = p_chr_percentage, width = 10, height = 8, dpi = 600)
