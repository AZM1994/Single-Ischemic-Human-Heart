...
############################### circos plot ####################################
################################################################################
Condition_list <- c("normal", "disease")
...
for (condition_temp in Condition_list){
  heart_PTA_Cases_vcf_temp <- read.table(paste0(project_dir, "/data/vcfs/heart_PTA_Cases.all_age.", condition_temp, "_ssnv.vcf"), sep = "\t")
  genomic_context_temp <- read.csv(paste0(project_dir, "/data/vcfs/heart_PTA_Cases.all_age.", condition_temp, ".annotation.csv"), header = TRUE) %>%
  ...
  genomic_context_temp <- genomic_context_temp %>%
    ...
    base::`[`(c("Chr", "Start", "Cell_ID", "Case_ID", "Condition")) %>% rename_with(~ c("Chr", "Postion", "Cell_ID", "Case_ID", "condition"))
