plot_spectrum_customize <- function (type_occurrences, CT = FALSE, by = NA, indv_points = FALSE, 
                                    ...
                                    legend = TRUE, condensed = FALSE) 
{
  ...
  tb <- tb_per_sample %>% dplyr::mutate(by = factor(by, levels = unique(by))) %>% 
    dplyr::group_by(by, variable) %>% dplyr::summarise(sub_type = sub_type[[1]], 
                                                       ...
                                                       total_mutations = round(sum(nmuts), digits=0)) %>% dplyr::mutate(total_individuals = sum(total_individuals), 
