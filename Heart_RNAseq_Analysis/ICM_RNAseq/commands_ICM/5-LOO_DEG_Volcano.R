...
wd_dir <- getwd()
LOO_DEG_Volcano_dir <- paste0(wd_dir, "/results/5-LOO_DEG_Volcano")
...
DEG_data <- read.csv(paste0(LOO_DEG_Volcano_dir, "/LOO_DEG_CM_only.csv")) %>% 
  select(-X) %>% 
...
DEG_df <- read.csv(paste0(LOO_DEG_Volcano_dir, "/LOO_DEG_up_down_df.csv"))
...
write.csv(DEG_up_down_df_withlogFC, paste0(LOO_DEG_Volcano_dir, "/DEG_up_down_df_withlogFC.csv"))
...
write.table(top_up_and_down_reg_gene_FC_pV, paste(LOO_DEG_Volcano_dir, '/top_up_and_down_reg_gene_FC_pV.csv', sep = ""),
...
DEG_vocalno <- ggplot(data = DEG_data_plot, aes(x = avg_log2FC, y = p_val_adj_log10, color = diffexpressed_plot, label = top_ten_label_mix_plot)) + 
  ...
  theme_linedraw() + theme(panel.background = element_rect(fill = "white"), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), 
                                           panel.border = element_rect(linewidth = 0.5), text = element_text(size = 20), legend.position = "none")
# DEG_vocalno
ggsave(paste0(LOO_DEG_Volcano_dir, "/DEG_vocalno.pdf"), plot = DEG_vocalno, width = 7.5, height = 4, dpi = 300)
