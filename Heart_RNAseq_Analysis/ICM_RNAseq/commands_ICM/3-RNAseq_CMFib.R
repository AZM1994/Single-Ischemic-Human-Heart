...
color_list_regroup <- c("#A65A34", "#FFB6C1", "#EE5A9B", "yellow", "#f22c4b", "#a50f15", "#FFC34D", "#e6550d",
                        "limegreen", "olivedrab", 
                        "dodgerblue3", "#a2d2e7", "#3F7CA7", "#084594", "dodgerblue1", "#b98fb9", "#9c9ede", "#944a94")
CM_Fibro_color <- c("#f22c4b", "#66B032", "#9c9ede")

##### set working and results directories
wd_dir <- getwd()
result_dir <- paste0(wd_dir, "/results_ICM")
Unsupervised_result_dir <- paste0(wd_dir, "/results_ICM/3-CMFib/Unsupervised_Analysis")
Supervised_result_dir <- paste0(wd_dir, "/results_ICM/3-CMFib/Supervised_Analysis")
...
############################## extract CM and Fib ##############################
################################################################################
# control.disease.combined <- readRDS(paste0(result_dir, "/0-Seurat_Object/Seurat.obj_with_annotation_all_celltypes.RDS"))
...
# control.disease.combined_CM_Fib <- RunPCA(control.disease.combined_CM_Fib, npcs = 20, verbose = TRUE) %>%
...
#   RunUMAP(reduction = "harmony", dims = 1:20, n.neighbors = 30, min.dist = 0.6, spread = 0.5, verbose = TRUE) %>% 
#   FindNeighbors(reduction = "harmony", dims = 1:20, verbose = TRUE) %>%
#   FindClusters(resolution = c(0.6, 0.8, 1.0, 1.2), verbose = TRUE)

# saveRDS(control.disease.combined_CM_Fib, paste0(wd_dir, "/results_ICM/0-Seurat_Object/Seurat.obj_extractCM_Fibro_ready_for_subclustering.RDS"))
control.disease.combined_CM_Fib  <- readRDS(paste0(wd_dir, "/results_ICM/0-Seurat_Object/Seurat.obj_extractCM_Fibro_ready_for_subclustering.RDS"))
...
unique(control.disease.combined_CM_Fib$RNA_snn_res.1.2)
# table_stat <- t(table(control.disease.combined_CM_Fib$RNA_snn_res.1.2, control.disease.combined_CM_Fib$samples))
# table_stat <- table_stat[unique(control.disease.combined_CM_Fib$samples), ]
level_list <- sort(unique(control.disease.combined_CM_Fib$RNA_snn_res.1.2))
control.disease.combined_CM_Fib$RNA_snn_res.1.2 <- factor(control.disease.combined_CM_Fib$RNA_snn_res.1.2, levels = level_list)

control.disease.combined_CM_Fib@meta.data <- control.disease.combined_CM_Fib@meta.data %>% 
  mutate(regroup = recode(RNA_snn_res.1.2, 
      "8" = "18", "7" = "17", "3" = "16", "4"  = "15", "13" = "14", "10" = "13", "5"  = "12", "14" = "11", "12" = "10", "9" = "9", 
      "15" = "0", "18" = "8", "11" = "7", "16" = "6", "6" = "5", "17" = "4", "2" = "3", "1" = "2", "0" = "1"), 
      regroup = factor(regroup, levels = level_list))

control.disease.combined_CM_Fib <- subset(control.disease.combined_CM_Fib, subset = regroup != 4)

level_list <- 0:17
control.disease.combined_CM_Fib@meta.data <- control.disease.combined_CM_Fib@meta.data %>% 
  mutate(regroup = recode(regroup,
                          "0" = "0", "1" = "1", "2" = "2", "3"  = "3", "5" = "4", "6" = "5", "7"  = "6", "8" = "7", "9" = "8", "10" = "9", 
...
pdf(paste0(Unsupervised_result_dir, "/Clustering/Unsupervised_clustering_cond_8_9.pdf"), width = 10, height = 5)
...
gene_group_df <- data.frame(gene = all_markers, group = marker_groups) %>% 
  mutate(group = factor(group, level = c("Cardiomyocyte markers", "Fibroblast markers")))
...
control.disease.combined_CM_Fib_Disease <- subset(control.disease.combined_CM_Fib, subset = condition == "ICM")

dotplot_all <- DotPlot(control.disease.combined_CM_Fib, features = all_markers, group.by = "regroup")
dotplot_all$data$id <- factor(dotplot_all$data$id, levels = as.character(level_list))
dotplot_all$data$gene_group <- factor(gene_group_df$group[match(dotplot_all$data$features.plot, gene_group_df$gene)])
dotplot_all <- dotplot_all + facet_grid(~ gene_group, scales = "free_x", space = "free_x") + scale_color_gradient(low = "white", high = "black") + scale_y_discrete(drop = FALSE) + 
...
dotplot_Control$data$id <- factor(dotplot_Control$data$id, levels = as.character(level_list))
...
dotplot_Control <- dotplot_Control + facet_grid(~ gene_group, scales = "free_x", space = "free_x") + scale_color_gradient(low = "black", high = "dodgerblue1") + scale_y_discrete(drop = FALSE) + 
  ...
  geom_tile(data = subset(dotplot_Control$data, id %in% c("8", "9")), fill = "yellow", color = "#FFFF004D", alpha = 0.65) + 
...
ggsave(paste0(Unsupervised_result_dir, "/Dotplot/Unsupervised_dotplot_Control.pdf"), plot = dotplot_Control, width = 10, height = 6.5, dpi = 600)

rm(dotplot_Disease)
dotplot_Disease <- DotPlot(control.disease.combined_CM_Fib_Disease, features = all_markers, group.by = "regroup")
dotplot_Disease$data$id <- factor(dotplot_Disease$data$id, levels = as.character(level_list))
...
dotplot_Disease <- dotplot_Disease + facet_grid(~ gene_group, scales = "free_x", space = "free_x") + scale_color_gradient(low = "black", high = "firebrick1") + scale_y_discrete(drop = FALSE) + 
  theme_linedraw() + theme(panel.background = element_rect(fill = "white"), panel.grid.major = element_blank(), text = element_text(size = 12), 
  ...
  guides(size = guide_legend(override.aes = list(color = "black", fill = "white")))
ggsave(paste0(Unsupervised_result_dir, "/Dotplot/Unsupervised_dotplot_Disease.pdf"), plot = dotplot_Disease, width = 10, height = 6.5, dpi = 600)
...
# saveRDS(control.disease.combined_CM_Fib, paste0(result_dir, "/0-Seurat_Object/Seurat.obj_with_annotation_extractCM_Fibro_filtered.RDS"))
# control.disease.combined_CM_Fib <- readRDS(paste0(result_dir, "/0-Seurat_Object/Seurat.obj_with_annotation_extractCM_Fibro_filtered.RDS"))

##### simple statistics
num_per_sample_cluster_unannotated <- table(control.disease.combined_CM_Fib$donor_condition, control.disease.combined_CM_Fib$regroup)
...
# marker_list <- c("DCN", "BICC1", "ABCA6")
# gene_list <- rownames(control.disease.combined_CM_Fib)
# marker_list %in% gene_list
# VlnPlot(control.disease.combined_CM_Fib, features = marker_list, idents = c(8, 9), split.by = "condition", ncol = 5)

gene_sets <- list(
  Cardiomyocyte_metagene = c("TNNT2", "ACTN2", "MYH6", "MYH7", "TNNI3"), 
  Fibroblast_metagene = c("DCN", "COL1A1", "BICC1", "FAP", "PDGFRA"), 
  Hypoxia_metagene = c("JUN", "VIM", "MCL1", "PDK1"), 
  Collagen_metagene = c("COL8A1", "COL12A1", "COL1A1"), 
  Fibrotic_metagene = c("POSTN", "FAP", "TGFBI", "YAP1", "TGFB1", "TGFB3")
)
...
all_metagene_grouped_avg <- metadata_CM_Fib %>% group_by(regroup, condition, samples) %>% 
  summarize(Cardiomyocyte = mean(Cardiomyocyte_metagene1, na.rm = TRUE), Fibroblast = mean(Fibroblast_metagene2, na.rm = TRUE), 
            Hypoxia = mean(Hypoxia_metagene3, na.rm = TRUE), Collagen = mean(Collagen_metagene4, na.rm = TRUE), 
            Fibrotic = mean(Fibrotic_metagene5, na.rm = TRUE))

all_metagene_grouped_avg_melt <- all_metagene_grouped_avg %>% 
  filter(regroup %in% c(8, 9)) %>%
  ...
  mutate(regroup.modified = factor(regroup.modified, levels = c("Cardiomyocyte subcluster 8", "Cardiomyocyte subcluster 9")))

pdf(paste0(Unsupervised_result_dir, "/metagene_expr_barplot_CMFib.pdf"), width = 23, height = 10)
  ggplot(all_metagene_grouped_avg_melt, aes(x = metagenes, y = avg_expr, color = condition)) + 
    ...
    scale_fill_manual(values = Ctrl_IHD_color) + scale_color_manual(values = Ctrl_IHD_color) + facet_wrap(. ~  regroup.modified) + 
    ...
    labs(x = "", y = "Normalized Metagene Expression")
dev.off()
...
pdf(paste0(Unsupervised_result_dir, "/metagene_CMFib_feature_plot.pdf"), width = 13, height = 20)
  ...
  p1 <- FeaturePlot(control.disease.combined_CM_Fib, features = "Cardiomyocyte_metagene1", cols = feature_plot_color) + ggtitle("Cardiomyocyte_metagene")
  p2 <- FeaturePlot(control.disease.combined_CM_Fib, features = "Fibroblast_metagene2", cols = feature_plot_color) + ggtitle("Fibroblast_metagene")
  p3 <- FeaturePlot(control.disease.combined_CM_Fib, features = "Hypoxia_metagene3", cols = feature_plot_color) + ggtitle("Hypoxia_metagene")
  p4 <- FeaturePlot(control.disease.combined_CM_Fib, features = "Collagen_metagene4", cols = feature_plot_color) + ggtitle("Collagen_metagene")
  p5 <- FeaturePlot(control.disease.combined_CM_Fib, features = "Fibrotic_metagene5", cols = feature_plot_color) + ggtitle("Fibrotic_metagene")
  umap_col <- p1 / p2 / p3 / p4 / p5
  v1 <- VlnPlot(control.disease.combined_CM_Fib, features = "Cardiomyocyte_metagene1", group.by = "regroup", cols = color_list_regroup, , pt.size = 0) + ggtitle("Cardiomyocyte_metagene") + theme(legend.position = "none")
  v2 <- VlnPlot(control.disease.combined_CM_Fib, features = "Fibroblast_metagene2", group.by = "regroup", cols = color_list_regroup, pt.size = 0) + ggtitle("Fibroblast_metagene") + theme(legend.position = "none")
  v3 <- VlnPlot(control.disease.combined_CM_Fib, features = "Hypoxia_metagene3", group.by = "regroup", cols = color_list_regroup, pt.size = 0) + ggtitle("Hypoxia_metagene")
  v4 <- VlnPlot(control.disease.combined_CM_Fib, features = "Collagen_metagene4", group.by = "regroup", cols = color_list_regroup, pt.size = 0) + ggtitle("Collagen_metagene") + theme(legend.position = "none")
  v5 <- VlnPlot(control.disease.combined_CM_Fib, features = "Fibrotic_metagene5", group.by = "regroup", cols = color_list_regroup, pt.size = 0) + ggtitle("Fibrotic_metagene") + theme(legend.position = "none")
...
annotated_clusters_list_CM <- c("Cardiomyocyte", "Cardiomyocyte", "Cardiomyocyte", "Cardiomyocyte", "Cardiomyocyte", "Cardiomyocyte", "Cardiomyocyte", "Cardiomyocyte", 
                                "Fibrotic-cardiomyocyte", "Fibrotic-cardiomyocyte", 
                                "Fibroblast", "Fibroblast", "Fibroblast", "Fibroblast", "Fibroblast", "Fibroblast", "Fibroblast", "Fibroblast")
...
# control.disease.combined_CM_Fib <- subset(control.disease.combined_CM_Fib, subset = annotated_clusters_CM %in% c("Cardiomyocyte", "Fibrotic-cardiomyocyte", "Fibroblast"))
...
pct_donor_cluster_annotated <- num_per_sample_cluster_annotated %>% 
  ...
  mutate(condition = c(rep("Control", 8), rep("ICM", 7)), ratio  = Cardiomyocyte / Fibroblast)

pct_long <- pct_donor_cluster_annotated %>% tibble::rownames_to_column("donor") %>% 
...
wilcox_test <- wilcox.test(ratio ~ condition, data = pct_donor_cluster_annotated)
p_value <- wilcox_test$p.value # 0.1519814
significance <- ifelse(p_value < 0.001, "***", ifelse(p_value < 0.01, "**", ifelse(p_value < 0.05, "*", "ns")))
max_y <- max(pct_long$Percentage) + 5

pdf(paste0(Supervised_result_dir, "/Dotplot/Supervised_cluster_prop_comparison.pdf"), width = 9, height = 5)
  ggplot(pct_long, aes(x = donor, y = Percentage, fill = Cell_Type)) + geom_bar(stat = "identity") + facet_wrap(~ condition, scales = "free_x") + labs(y = "Percentage", x = "Donor", fill = "Cell Type") + 
    ...
    theme(panel.background = element_rect(fill = "white"), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), text = element_text(size = 16), panel.border = element_rect(size = 0.5), axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1))
dev.off()
...
pdf(paste0(Supervised_result_dir, "/Dotplot/Cell_pct_per_celltype.pdf"), width = 10, height = 8)
...
##### save object
# saveRDS(control.disease.combined_CM_Fib, paste0(result_dir, "/0-Seurat_Object/Seurat.obj_sub_clustering_with_annotation_final.RDS"))
