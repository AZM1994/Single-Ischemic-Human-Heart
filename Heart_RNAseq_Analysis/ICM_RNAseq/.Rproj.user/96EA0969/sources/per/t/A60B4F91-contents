...
wd_dir <- getwd()
LOO_DEG_GOseq_dir <- paste0(wd_dir, "/results/6-LOO_DEG_GOseq")
...
##### read DEGs
DEG_df <- read.csv(paste0(LOO_DEG_GOseq_dir, "/LOO_DEG_up_down_df.csv"))
DEG_up <- DEG_df$Gene[DEG_df$regulation == "up"]
DEG_down <- DEG_df$Gene[DEG_df$regulation == "down"]
### read RNAseq CM data
Seurat.obj_sub_clustering_CM_only <- readRDS(paste0(wd_dir, "/results/0-Seurat_Object/Seurat.obj_sub_clustering_CM_only.RDS"))
...
write.csv(GO_BP_all, paste0(LOO_DEG_GOseq_dir, "/GO_BP_all.csv"))

pdf(paste0(LOO_DEG_GOseq_dir, "/DEG_GO_volcano_Up.pdf"), width = 15, height = 10)
...
pdf(paste0(LOO_DEG_GOseq_dir, "/DEG_GO_volcano_Down.pdf"), width = 13, height = 10)
