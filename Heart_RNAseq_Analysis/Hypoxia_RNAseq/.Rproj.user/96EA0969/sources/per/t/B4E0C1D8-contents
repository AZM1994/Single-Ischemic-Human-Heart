...
color_list <- c("red", "#2ca02c", "dodgerblue", "#ff7f0e", "#9467bd", "#e377c2", "#8c564b", "#7f7f7f", "#bcbd22", "#17becf",
                ...
                "#e7ba52", "#e7969c", "#d6616b", "#ad494a", "#843c39", "#fdd0a2", "#fb6a4a", "#cb181d", "#a50f15",
...
wd_dir <- getwd()
Unsupervised_result_dir <- paste0(wd_dir, "/results_IHD/2-All_Celltypes/Unsupervised_Analysis")
Supervised_result_dir <- paste0(wd_dir, "/results_IHD/2-All_Celltypes/Supervised_Analysis")
...
######################################################################################################
######################################### continue from O2 ###########################################
control.disease.combined  <- readRDS(paste0(wd_dir, "/results_IHD/0-Seurat_Object/combined.Seurat.obj_ready_for_annotation.RDS"))
selected_sample_names <- unique(control.disease.combined$samples)
selected_donor_names <- unique(control.disease.combined$donor)
...
level_list <- sort(unique(control.disease.combined$RNA_snn_res.1))
control.disease.combined$RNA_snn_res.1 <- factor(control.disease.combined$RNA_snn_res.1, levels = level_list)
...
pdf(paste0(Unsupervised_result_dir, "/Clustering/Unsupervised_clustering_donor.pdf"), width = 28, height = 4)
  DimPlot(control.disease.combined, reduction = 'umap', split.by = "donor", label = TRUE, label.size = 3, cols = color_list, order = T) + 
...
Endo_markers <- c("VWF", "PECAM1", "FABP5", "ID1") # Endothelial
...
Mast_markers <- c("IL18R1", "KIT", "CPA3") # Mast

all_markers <- c(CM_markers, Fib_markers, Endo_markers, Pericyte_markers, Neuronal_markers, Immune_markers, Mast_markers)
marker_groups <- c(rep("Cardiomyocyte markers", length(CM_markers)), rep("Fibroblast markers", length(Fib_markers)), rep("Endothelial markers", length(Endo_markers)), rep("Pericyte markers", length(Pericyte_markers)), 
                   rep("Neuronal markers", length(Neuronal_markers)), rep("Immune markers", length(Immune_markers)), rep("Mast markers", length(Mast_markers)))
...
ggsave(paste0(Unsupervised_result_dir, "/Dotplot/Unsupervised_dotplot_all.pdf"), plot = dotplot_all, width = 13, height = 8, dpi = 300)
...
# control.disease.combined@meta.data$annotated_clusters <- NULL

##### Reorder the cell types in the metadata
cell_type_order <- c("Cardiomyocyte", "Fibroblast", "Endothelial", "Pericyte", "Neuronal", "Immune", "Immune-Mast")
...
pdf(paste0(Supervised_result_dir, "/Clustering/Supervised_clustering_sample.pdf"), width = 50, height = 4)
  DimPlot(control.disease.combined, reduction = 'umap', group.by = "annotated_clusters", split.by = "samples", label = T, label.size = 3, cols = color_list_02, order = T) + 
    facet_grid(. ~  factor(samples, level = selected_sample_names), scale = "free_x") + ggtitle(NULL) + theme_linedraw() + 
    theme(panel.background = element_rect(fill = "white"), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), text = element_text(face = "bold", size = 12))
dev.off()

pdf(paste0(Supervised_result_dir, "/Clustering/Supervised_clustering_sample_donor.pdf"), width = 28, height = 4)
  DimPlot(control.disease.combined, reduction = 'umap', group.by = "annotated_clusters", split.by = "donor", label = T, label.size = 3, cols = color_list_02, order = T) + 
    facet_grid(. ~  factor(donor, level = selected_donor_names), scale = "free_x") + ggtitle(NULL) + 
    theme_linedraw() + theme(panel.background = element_rect(fill = "white"), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), text = element_text(face = "bold", size = 12))
dev.off()
...
num_per_donor_cluster_annotated <- table(control.disease.combined$donor, control.disease.combined$annotated_clusters)
num_per_donor_cluster_annotated <- num_per_donor_cluster_annotated[as.character(unique(control.disease.combined$donor)), ]
...
pct_sample_cluster_annotated_melt <- pct_sample_cluster_annotated %>% melt() %>% 
...
pdf(paste0(Supervised_result_dir, "/Dotplot/Supervised_cluster_prop_comparison.pdf"), width = 20, height = 6)
  barplot_annotated_cluster_ratio <- ggbarplot(pct_sample_cluster_annotated_melt, x = "cluster", y = "ratio", color = "condition", 
                                              label = TRUE, lab.nb.digits = 2, add = c("mean_se", "jitter"), position = position_dodge(0.9), palette = c("dodgerblue4", "firebrick4")) +
    stat_compare_means(aes(group = condition), method = "wilcox", label = "p.format", label.y = 1.02 * max(pct_sample_cluster_annotated_melt$ratio, na.rm = TRUE)) + 
    stat_compare_means(aes(group = condition), method = "wilcox", label = "p.signif", label.y = 1.06 * max(pct_sample_cluster_annotated_melt$ratio, na.rm = TRUE)) + 
...
melt_pct_donor_cluster_annotated <- melt(as.matrix(pct_donor_cluster_annotated[, -8])) %>% 
  mutate(condition = ifelse(Var1 %in% selected_donor_names[1:3], "Control", ifelse(Var1 %in% selected_donor_names[4:8], "IHD", value)))
...
##### save object
# saveRDS(control.disease.combined, paste0(wd_dir, "/results_IHD/0-Seurat_Object/Seurat.obj_with_annotation_all_celltypes.RDS"))
