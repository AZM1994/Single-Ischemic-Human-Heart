...
wd_dir <- getwd()
result_dir <- paste0(wd_dir, "/results_IHD/3-CMFib_Trajectory")
...
# root_cells <- rownames(meta %>% filter(annotated_clusters_CM == "Cardiomyocyte") %>% arrange(desc(Cardiomyocyte_metagene)) %>% head(20))
...
#   trajectory_graph_segment_size = 1.25, cell_size = 0.5) + scale_color_viridis_c(option = "magma", limits = c(0, 25), direction = -1)
# ggsave(paste0(result_dir, "/p_pseudotime_all.pdf"), plot = p_pseudotime_all, width = 10, height = 8, dpi = 300)
...
plot_condition_trajectory <- function(cds, condition_label, keep_all) {
  ...
  plot_cells(cds_sub, color_cells_by = "pseudotime", label_groups_by_cluster = FALSE, trajectory_graph_color = "#00FFFF", label_leaves = FALSE, 
    ...
    scale_x_continuous(limits = c(-10, 10), breaks = c(-5, 0, 5, 10)) + scale_y_continuous(limits = c(-8, 7), breaks = c(-8, -4, 0, 4)) + theme_linedraw() + 
...
p_ihd  <- plot_condition_trajectory(cds, "IHD", keep_all = TRUE)
p_cond <- p_ctrl + p_ihd
