...
def baseline_guess_to_combined_finetuner_guess(model, classifier, data):
    ...
    import gc
    with torch.no_grad():
        data.classification = classifier(mutation_dist=data.inputs, num_mut=data.num_mut)
        # data.inputs = data.inputs[data.classification.view(-1) > 0.5, ]
        data.inputs = torch.where(data.classification > 0.5, data.inputs, torch.tensor(0))
        # data.num_mut = data.num_mut[data.classification > 0.5, ].reshape(-1,1)
        data.num_mut = torch.where(data.classification > 0.5, data.num_mut, torch.tensor(0))
        # data.prev_guess = data.prev_guess[data.classification.view(-1) > 0.5]
