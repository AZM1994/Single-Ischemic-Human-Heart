...
class ClassifiedFinetunerErrorfinder:
    ...
    def __join_and_sort(self, realistic, random, ind_order):
        ...
        # print("realistic", realistic.size())
        # print("ind_order", ind_order.size())
    ...
    def __call__(self,
        ...
        upper, lower = self.errorfinder(weights=finetuner_guess_realistic,
                                                num_mutations=num_mut_realistic,
                                                classification=classification_realistic.reshape(-1, 1))
        upper = self.__join_and_sort(upper, torch.full_like(baseline_guess_random, float('nan')), ind_order)
        lower = self.__join_and_sort(lower, torch.full_like(baseline_guess_random, float('nan')), ind_order)
