...
# all_gene_normal <- gene_length_deduped$Gene_symbol %in% genomic_context_normal$Gene_symbol[genomic_context_normal$Type == type_list]
...
# all_gene_normal_GO_filtered_with_genes <- all_gene_normal_GO_filtered
...
# all_gene_disease_GO_filtered_with_genes <- all_gene_disease_GO_filtered
# all_gene_disease_GO_filtered_with_genes <- rbind(all_gene_normal_GO_filtered, all_gene_disease_GO_filtered) %>% 
#   mutate(over_represented_pvalue_corrected = p.adjust(over_represented_pvalue, method = "fdr"))
...
all_GO_filtered_with_genes_melt_01 <- all_GO_filtered_with_genes_melt[all_GO_filtered_with_genes_melt$ontology == c("BP"),] %>% 
  mutate(Condition = factor(Condition, level = c("Normal", "IHD")))
...
pdf("GO_results/all_gene_GO_dotplot.pdf", width = 20, height = 15)
...
pdf("GO_results/all_gene_GO_volcano.pdf", width = 14, height = 10)
