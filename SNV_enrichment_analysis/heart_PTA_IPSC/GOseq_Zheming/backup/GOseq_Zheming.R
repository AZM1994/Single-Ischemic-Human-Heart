...
perform_GOseq_analysis <- function(type_list, normal_raw_tsv_name, disease_raw_tsv_name, 
                                   permutation_table_name, go_table_name, figure_name, figure_name_png, gene_length_type){
  ...
  ready <- melt(go_table[go_table$corrected_p_permutation < 0.05|go_table$corrected_p_normal < 0.05, c(1,11,13,14)])
  colnames(ready) <- c("GO", "Term", "Variable", "Pvalue")
  ready$Term <- factor(ready$Term, levels = rev(unique(ready$Term)))
  ready$Condition <- factor(sapply(ready$Variable, function(x){
    if(grepl("normal", x)){
  ...
  ggsave(paste0(figure_name_png, "_03.png"),
         plot = p3, width = 8, height = 10, dpi = 300)
  
  dev.off()
}

permutation_round = 3
numDEInCat_threshold = 5
numInCat_threshold = 500
...
perform_GOseq_analysis(c("exonic"), c("GO_results/normal.exonic_only.GO.tsv"), c("GO_results/disease.exonic_only.GO.tsv"), 
                       c("GO_results/disease_vs_normal.exonic_only.raw.tsv"), c("GO_results/disease_vs_normal.exonic_only.GO.tsv"), 
                       c("GO_results/disease_vs_normal.exonic_only.GOseq.pdf"), c("GO_results/disease_vs_normal.exonic_only.GOseq"), 
                       "Exon_length")

##### GOseq for both exonic and intronic
perform_GOseq_analysis(c("exonic", "intronic"), c("GO_results/normal.exonic_and_intronic.GO.tsv"), c("GO_results/disease.exonic_and_intronic.GO.tsv"),
                       c("GO_results/disease_vs_normal.exonic_and_intronic.raw.tsv"), c("GO_results/disease_vs_normal.exonic_and_intronic.GO.tsv"),
                       c("GO_results/disease_vs_normal.exonic_and_intronic.GOseq.pdf"), c("GO_results/disease_vs_normal.exonic_and_intronic.GOseq"),
                       "Gene_length")
