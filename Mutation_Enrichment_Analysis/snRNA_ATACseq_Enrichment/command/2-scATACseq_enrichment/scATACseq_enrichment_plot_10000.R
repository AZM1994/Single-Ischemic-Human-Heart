...
fig_save_dir <- paste0("./results/ATAC_analysis_by_case/", analysis_ID, "_final/")
suppressWarnings(dir.create(fig_save_dir, recursive = TRUE))

##### read in all mutmat
mut_mat_dir <- paste0("./results/ATAC_analysis_by_case/", analysis_ID, "_mut_mat/")
...
# n_split <- ceiling(nrow(permutation_mut_mat_all_raw) / 20)
# for (i in 1:20) {
#   start_row <- (i - 1) * n_split + 1
#   end_row <- min(i * n_split, nrow(permutation_mut_mat_all_raw))
#   cat(i, start_row, ":", end_row, "\n")
#   chunk <- permutation_mut_mat_all_raw[start_row:end_row, ]
#   suppressWarnings(dir.create(paste0(fig_save_dir, "permutation_mut_mat_all_raw_split"), recursive = TRUE))
#   write.csv(chunk, file = paste0(fig_save_dir, "permutation_mut_mat_all_raw_split/permutation_mut_mat_all_raw_part", i, ".csv"))
#   }
# 
# n_split <- ceiling(nrow(permutation_mut_mat_all_est) / 20)
# for (i in 1:20) {
#   start_row <- (i - 1) * n_split + 1
#   end_row <- min(i * n_split, nrow(permutation_mut_mat_all_est))
#   cat(i, start_row, ":", end_row, "\n")
#   chunk <- permutation_mut_mat_all_est[start_row:end_row, ]
#   suppressWarnings(dir.create(paste0(fig_save_dir, "permutation_mut_mat_all_est_split"), recursive = TRUE))
#   write.csv(chunk, file = paste0(fig_save_dir, "permutation_mut_mat_all_est_split/permutation_mut_mat_all_est_part", i, ".csv"))
# }
...
p_SNV_ATACseq_enrichment <- ggplot(merged_summary_plot, aes(x = decile, y = mean_ER, group = Condition, color = Condition)) + 
  ...
  annotate(geom = "polygon", x = c(1, group_num, group_num), y = c(0.8, 0.8, 0.8 + 0.04), fill = "#28a745") + 
  ...
  labs(x = "Chromatin accessibility level", y = "sSNV enrichment ratio \n (obs/exp)", color = "Condition", subtitle = overall_label)
ggsave(paste0(fig_save_dir, "/sSNV_snATACseq_Enrichment.pdf"), plot = p_SNV_ATACseq_enrichment, width = 9, height = 5, dpi = 600)
...
# selected_sigs <- all_sig_contri %>% filter(Ctrl_pct >= sig_pct_thr | Dis_pct >= sig_pct_thr)
# selected_sigs_ctrl <- selected_sigs$Ctrl_pct >= sig_pct_thr
# selected_sigs_dis <- selected_sigs$Dis_pct >= sig_pct_thr
...
for (sig_id_index in 1){
  ...
  # filter(Condition %in% c("Control", "IHD")[c(selected_sigs_ctrl[sig_id_index], selected_sigs_dis[sig_id_index])])
  ...
  p_enrichment_SigNet <- ggplot(SigNet_contri_temp, aes(x = decile, y = mean_EnR, color = Condition, group = Condition)) + 
    ...
    scale_fill_manual(values = c("Control" = color_set[1], "IHD" = color_set[2]), guide = "legend") + theme_linedraw() + 
    theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.border = element_rect(linewidth = 0.5), text = element_text(size = 24)) + 
    # annotate(geom = "polygon", x = c(1, group_num, group_num), y = c(0.1, 0.1, 0.1 + 0.15), fill = "#28a745") +
    # scale_y_continuous(breaks = seq(0.0, 2.0, by = 0.5), limits = c(0.1, 2.3)) +
    annotate(geom = "polygon", x = c(1, group_num, group_num), y = c(0.3, 0.3, 0.3 + 0.1), fill = "#28a745") +
    scale_y_continuous(breaks = seq(0.6, 1.6, by = 0.2), limits = c(0.3, 1.8)) +
    labs(x = "Chromatin accessibility level", y = paste0(sig_id, " enrichment ratio \n (obs/exp)"), color = "Condition", subtitle = overall_label)
  ggsave(paste0(fig_save_dir, sig_id, "_snATACseq_Enrichment.pdf"), plot = p_enrichment_SigNet, width = 9, height = 5)
}