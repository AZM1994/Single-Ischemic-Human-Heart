...
gene_chr_expr_normalize <- merge(gene_chr_expr,  Control_median_df, by = "chr") %>% mutate(average_expr_level_nor = average_expr_level / median_expr)

t_test_results <- gene_chr_expr_normalize %>% group_by(chr) %>%
  summarize(p_value = t.test(average_expr_level_nor ~ Condition)$p.value)
gene_chr_expr_normalize <- gene_chr_expr_normalize %>% left_join(t_test_results, by = "chr")
# a=as.data.frame(table(gene_chr_expr$average_expr_level))
# b1= gene_chr_expr_normalize$average_expr_level_nor[gene_chr_expr_normalize$Condition == "Control" & gene_chr_expr_normalize$chr == "chr11"]
# b2= gene_chr_expr_normalize$average_expr_level_nor[gene_chr_expr_normalize$Condition == "IHD" & gene_chr_expr_normalize$chr == "chr11"]
# 
# wilcox.test(b1,b2)
...
p_gene_chr_expr_normalize <- ggplot(gene_chr_expr_normalize, aes(x = Condition, y = average_expr_level_nor, fill = Condition)) + 
  ...
  # stat_compare_means(aes(group = Condition), method = "t.test", label = "p.format", label.y = 1.02 * max(gene_chr_expr_normalize$median_expr)) +
  # stat_compare_means(aes(group = Condition), method = "t.test", label = "p.signif", label.y = 1.06 * max(gene_chr_expr_normalize$median_expr)) +
