...
O2_results_dir <- paste0("./data/permutation_RNAseq/1000P_", group_num, "G/")
...
p_enrichment_RNAseq_snv_filtered <- ggplot(merged_summary_plot, aes(x = decile, y = mean_ER, group = Condition, color = Condition)) + 
  ...
  annotate(geom = "polygon", x = c(1, 8, 8), y = c(0.45, 0.45, 0.53), fill = "#2ca02c", alpha = 0.8) + 
  ...
  ylim(c(0.45, 1.5)) + labs(x = "Gene expression levels", y = "sSNV enrichment ratio \n (obs/exp)", color = "Condition", title = "")
ggsave(paste0(fig_save_dir, "/enrichment_RNAseq_snv_filtered.pdf"), plot = p_enrichment_RNAseq_snv_filtered, width = 9, height = 5, dpi = 600)
