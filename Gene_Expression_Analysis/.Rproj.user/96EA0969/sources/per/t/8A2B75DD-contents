...
gene_chr_access_normalize <- merge(gene_chr_access,  Control_median_df, by = "chr") %>% 
  mutate(score_nor = round(score / median_access, 3)) %>% 
  mutate(score_nor = ifelse(score_nor == 0, 0.001, score_nor))

wilcox_test_results <- gene_chr_access_normalize %>% group_by(chr) %>%
  summarize(p_value = wilcox.test(score_nor ~ Condition)$p.value)
gene_chr_access_normalize <- gene_chr_access_normalize %>%
  left_join(wilcox_test_results, by = "chr") |> base::`[`(c("chr", "score_nor", "Condition", "p_value"))
  # mutate(score_nor = score_nor + 1)
  # mutate(score_nor = score_nor^(1/6))
...
p_gene_chr_access_normalize <- ggplot(gene_chr_access_normalize, aes(x = Condition, y = score_nor, fill = Condition)) + 
  ...
  # geom_text(data = gene_chr_access_normalize, aes(x = 1.5, y = max(score_nor), label = paste0("p = ", round(p_value, 2))), vjust = 2, size = 8, nudge_y = 1.5) + 
...
# ggsave(paste0(fig_save_dir, "violin_gene_chr_access_normalize1.png"), p_gene_chr_access_normalize, width = 20, height = 10,  dpi = 300)
